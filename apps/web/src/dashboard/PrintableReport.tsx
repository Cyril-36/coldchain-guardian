...
export function PrintableReport({ run, snapshot, report, reviewOverride }: { run: Run; snapshot: Snapshot; report: Report; reviewOverride?: Review | null }) {
  ...
  return <section className="print-report" aria-labelledby="print-report-title">
    ...
    <section className="print-section"><h2>Measurements</h2><div className="print-grid"><div><strong>Observed maximum</strong><span>{measurement?.observed_max_c == null ? "—" : `${measurement.observed_max_c}°C`}</span></div><div><strong>Estimated time out of range</strong><span>{formatDuration(measurement?.estimated_out_of_range_seconds)}</span></div><div><strong>Coverage</strong><span>{measurement?.coverage_status ?? "—"}</span></div><div><strong>Unknown duration</strong><span>{formatDuration(measurement?.unknown_duration_seconds)}</span></div></div></section>
