...
export function useRunData(): RunData {
  ...
  useEffect(() => {
    ...
    if (!auth.authenticated) {
      setData((current) => ({ ...current, protectedRun: true, source: "api", loading: false, snapshotReady: false, reportReady: false, error: null, retry }));
      return;
    }
    ...
    const hydrateArtifacts = async (currentRun: Run) => {
      const [snapshotResult, reportResult] = await Promise.allSettled([api.getSnapshot(runId), api.getReport(runId)]);
      if (cancelled) return;
    ...
    const poll = async () => {
      ...
      try {
        ...
        const delay = Date.now() - startedAt >= BACKOFF_AFTER_MS ? BACKOFF_POLL_MS : ACTIVE_POLL_MS;
    ...
    const initialise = async () => {
      ...
      try {
        ...
        if (cancelled || isTerminal(run.status)) return;
        pollTimer = window.setTimeout(() => void poll(), ACTIVE_POLL_MS);
      } catch (error: unknown) {
        if (!cancelled) setData((current) => ({ ...current, source: "api", protectedRun: true, snapshotReady: false, reportReady: false, loading: false, error: error instanceof Error ? error : new Error("Unable to load investigation data"), retry }));
      }
    };
