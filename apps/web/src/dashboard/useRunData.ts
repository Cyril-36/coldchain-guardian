...
export function useRunData(): RunData {
  ...
  useEffect(() => {
    ...
    const hydrateArtifacts = async (currentRun: Run) => {
      const [snapshotResult, reportResult] = await Promise.allSettled([api.getSnapshot(protectedRunId), api.getReport(protectedRunId)]); if (cancelled) return;
    ...
    const poll = async () => {
      ...
      try {
        const run = await api.getRun(protectedRunId); if (cancelled) return;
