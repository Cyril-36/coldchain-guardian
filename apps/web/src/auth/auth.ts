...
function config(): UserManagerSettings | null {
  ...
  return {
    ...
    response_type: "code",
    scope: "openid email coldchain/write",
...
export async function beginSignIn() {
  ...
  await getManager().signinRedirect({ state: current });
}

export async function completeSignIn(): Promise<User> {
  const user = await getManager().signinCallback();
  const returnTo = typeof user.state === "string" && user.state.startsWith("/") ? user.state : sessionStorage.getItem(RETURN_TO_KEY) ?? "/";
