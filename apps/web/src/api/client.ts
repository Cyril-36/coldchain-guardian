...
const reportSchema = z.object({
  report_id: uuid, run_id: uuid, snapshot_id: uuid, snapshot_sha256: z.string(), schema_version: schemaVersion, detector_version: z.string(), prompt_version: z.string(), model_id: z.string().nullable(), created_at: iso, cutoff_at: iso,
  measurements: z.array(z.object({ sensor_id: uuid, first_observed_out_at: iso.nullable(), last_observed_out_at: iso.nullable(), estimated_out_of_range_seconds: z.number().finite(), unknown_duration_seconds: z.number().finite(), sample_count: z.number().int().nonnegative(), observed_min_c: z.number().finite().nullable(), observed_max_c: z.number().finite().nullable(), censored_start: z.boolean(), censored_end: z.boolean(), coverage_status: z.enum(["complete", "partial", "insufficient"]), evidence_ids: z.array(uuid) }).strict()),
  outcome: z.enum(["hypothesis_supported", "unresolved", "no_excursion"]), primary_hypothesis: hypothesis.nullable(),
  hypotheses: z.array(z.object({ hypothesis, assessment: z.enum(["supported", "contradicted", "insufficient"]), supporting_evidence_ids: z.array(uuid), conflicting_evidence_ids: z.array(uuid), missing_evidence: z.array(z.string()), explanation: z.string() }).strict()),
  next_checks: z.array(z.object({ code: z.enum(["inspect_door", "check_refrigeration", "verify_sensor", "request_missing_logs", "quality_review"]), reason: z.string(), related_evidence_ids: z.array(uuid) }).strict()),
  limitations: z.array(z.string()), verification: z.object({ status: z.enum(["passed", "blocked"]), errors: z.array(z.string()), warnings: z.array(z.string()) }).strict(), generation_mode: z.enum(["bedrock", "deterministic_only"]), review_required: z.boolean(), evidence: z.array(evidence),
}).strict();
...
export class ApiClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) throw new ApiClientError(response.status, isApiErrorEnvelope(body) ? body.error : null, typeof body === "string" ? body : undefined);

    const schema = schemaFor(path, options.method ?? "GET");
    if (schema) {
      const result = schema.safeParse(body);
      if (!result.success) throw new Error(`Invalid API response for ${path}: ${result.error.issues[0]?.message ?? "schema validation failed"}`);
